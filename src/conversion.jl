function baseModelica_to_ModelingToolkit(model::BaseModelicaModel)
    ...
    var_funcs = [only(@variables $name(t) [description = "$desc"]) for (name,desc) in zip(var_names,var_descs)]
    ...
    param_defaults = parameters .=> [param.value == "" ? nothing : parse(Float64,param.value) for param in model.parameters]

    init_subst_dict = Dict(vcat(param_defaults,variables .=> var_funcs))

    init_lhs_exprs = [substitute(Symbolics.parse_expr_to_symbolic(Meta.parse(lhs),Main),init_subst_dict) for lhs in init_lhses]
    init_rhs_exprs = [substitute(Symbolics.parse_expr_to_symbolic(Meta.parse(lhs),Main),init_subst_dict) for lhs in init_rhses]
    ...
    lhs_exprs = [substitute(Symbolics.parse_expr_to_symbolic(Meta.parse(lhs),BaseModelica),subst_dict) for lhs in equ_lhses]
    rhs_exprs = [substitute(Symbolics.parse_expr_to_symbolic(Meta.parse(rhs),BaseModelica),subst_dict) for rhs in equ_rhses]

    eqs = [lhs ~ rhs for (lhs,rhs) in zip(lhs_exprs, rhs_exprs)]

    model_name = Symbol(model.name)
    sys = structural_simplify(ODESystem(eqs, t, var_funcs, parameters; defaults = defaults_and_inits, name = model_name))
end
